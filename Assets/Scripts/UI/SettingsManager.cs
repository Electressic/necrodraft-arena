...
public class SettingsManager : MonoBehaviour
{
    ...
    // Store the Unity default values for reset
    ...
    void Start()
    {
        // Store Unity slider defaults before any changes
        StoreUnityDefaults();
    ...
    {
        // Configure sliders for 0-100 percentage range
    ...
    public void SetMasterVolume(float volume)
    {
        if (audioMixer != null)
        {
            // Convert linear slider value (0-1) to decibel scale (-80 to 0)
            float dbValue = volume > 0 ? Mathf.Log10(volume) * 20 : -80f;
            audioMixer.SetFloat("MasterVolume", dbValue);
        }
        
        // If no audio mixer, directly control AudioListener
    ...
    public void SetMusicVolume(float volume)
    {
        ...
        // Update MusicManager if it exists (using SendMessage to avoid compile dependency)
    ...
    {
        // Reset to the original Unity slider values, not hardcoded defaults
        if (masterVolumeSlider != null)
        {
